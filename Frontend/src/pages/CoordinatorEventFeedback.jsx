...
export default function CoordinatorEventFeedback() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const assignedEvents = extractEventList(assignedResponse.data);
        const event = assignedEvents.find((item) => normalizeId(item?._id) === normalizeId(eventId));
  ...
  return (
    <div className="eventmate-page min-h-screen bg-slate-100/80 dark:bg-gray-900 px-4 sm:px-6 py-8">
      <div className="max-w-6xl mx-auto space-y-6">
        <button
          type="button"
          onClick={() => navigate(`/coordinator-dashboard/event/${encodeURIComponent(eventId || "")}/registrations`)}
