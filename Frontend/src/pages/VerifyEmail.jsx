...
export default function VerifyEmail() {
  ...
  useEffect(() => {
    const infoMessage = String(location.state?.message || "").trim();
    if (!infoMessage) return;
    emitToast({ type: "info", text: infoMessage });
  }, [location.state]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.email || !formData.otp) {
      emitToast({ type: "error", text: "Email and OTP are required." });
      return;
    }
    ...
    try {
      ...
      emitToast({
        type: "error",
        text: error.response?.data?.message || error.message || "Verification failed. Please try again.",
      });
    } finally {
      setIsLoading(false);
    }
  ...
  const handleResendOtp = async () => {
    if (!formData.email) {
      emitToast({ type: "error", text: "Enter your email to resend the OTP." });
      return;
    }
    ...
    try {
      ...
      emitToast({
        type: "error",
        text: error.response?.data?.message || error.message || "Unable to resend the OTP right now.",
      });
    } finally {
